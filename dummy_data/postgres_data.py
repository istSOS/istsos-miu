...
#                 for row in csv_data:
#                     #insert_query = f"INSERT INTO {schema_name}.{table} VALUES ({','.join(['%s'] * len(row))})"
                    
#                     # print([ type(z) for z in row ])
#                     insert_query = f"""INSERT INTO {schema_name}.{table} VALUES ({','.join([ "'"+str(z)+"'" if str(z)!="NULL" else 'null' for z in row ] )})"""
